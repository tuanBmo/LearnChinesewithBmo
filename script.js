...
let quizData = [];
let grammarData = []; // Chứa dữ liệu ngữ pháp động từ tệp grammar.csv
let currentIndex = 0;
let hp = 5;
let missedWords = []; 
...
const motivationalQuotes = [
    ...
    { hanzi: "学 như nghịch thủy hành chu, bất tiến tắc thối。", pinyin: "Xué rú nì shuǐ xíng zhōu, bù jìn zé tuì.", meaning: "Học như thuyền ngược dòng, không tiến ắt lùi." },
    ...
    { hanzi: "万事开头难。", pinyin: "Wàn饰 kāi tóu nán.", meaning: "Vạn sự khởi đầu nan." }
];
...
if(searchInput) {
    ...
    document.addEventListener('click', (e) => {
        if(!searchInput.contains(e.target) && !searchBox.contains(e.target)) searchBox.style.display = 'none';
    });
}
...
// ==========================================
// 11. TẢI VÀ RENDER NGỮ PHÁP ĐỘNG TỪ CSV
// ==========================================
        ...
        if (response.ok) {
            ...
            document.getElementById('grammarListContainer').innerHTML = `
                <p class="muted" style="text-align:center; padding: 20px;">
                    ...
                    Chưa tìm thấy tệp <strong>grammar.csv</strong> ở thư mục gốc của bạn. Hãy tạo tệp để kích hoạt tủ sách ngữ pháp!
                </p>`;
        }
...
window.changeGrammarLevel = function(level, btn) {
    document.querySelectorAll('#grammarLevelGroup .mode-btn').forEach(b => b.classList.remove('active'));
...
function renderGrammar(level) {
    ...
    // Lọc dòng có cột 2 (Cột Quyển) trùng khớp
    const filtered = grammarData.filter(row => {
        const rLvl = row[1] ? row[1].toString().trim().toLowerCase() : "";
        return rLvl === level.toLowerCase() || rLvl.includes('hsk' + level) || rLvl.includes('quyển ' + level);
    });
    ...
    container.innerHTML = filtered.map((row, idx) => {
        ...
        return `
            <div class="grammar-item">
                ...
                <p class="muted" style="margin-bottom: 15px; color: var(--text-secondary);">${explanation}</p>
...
function checkAuth() {
    ...
    if (!currentUser) {
        ...
        initSettingsUI(); renderDailyQuote(); loadGrammarData(); // Load ngữ pháp động
    }
}
...
// ==========================================
// 13. TABS NAVIGATION & CORE LOGIC GAME (Giữ nguyên vẹn cũ)
// ==========================================
...
function renderPersonalFiles() {
    ...
    Object.keys(personalFiles).forEach(name => {
        ...
        item.innerHTML = `<h4><i class='bx bx-file' style="color: var(--accent-sky-text); font-size:1.5rem;"></i> ${name}.csv</h4><button class="btn-delete" onclick="deleteFile('${name}')">Xóa</button>`;
        list.appendChild(item);
...
// ==========================================
// 14. MINI GAME LOGIC & BUBBLES HANDLER
// ==========================================
...
// ==========================================
// 15. LOGIC GAME LUYỆN TẬP CHÍNH & SENTENCE MINING
// ==========================================
...
function showQuestion() {
    ...
    if (selectedMode === "GÕ PINYIN") {
        mainQ.innerText = hanzi; mainQ.style.fontSize = "7.5rem"; hiddenText = meaning; 
        ...
        if (selectedMode === "NGHĨA") { mainQ.innerText = hanzi; mainQ.style.fontSize = "7.5rem"; hiddenText = pinyin; correctAnswer = meaning; }
        else if (selectedMode === "PINYIN") { mainQ.innerText = hanzi; mainQ.style.fontSize = "7.5rem"; hiddenText = meaning; correctAnswer = pinyin; }
        else { mainQ.innerText = meaning; mainQ.style.fontSize = "4rem"; hiddenText = pinyin; correctAnswer = hanzi; answerContainer.classList.add('hanzi-mode'); }
